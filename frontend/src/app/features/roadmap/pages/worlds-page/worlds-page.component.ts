...
import type { Biome, Section } from '../../data-access/educa/models';
import { EducaBadgeComponent } from '../../ui/educa-ui/educa-ui.component';
import { VisitService } from '../../data-access/educa/visit.service';
import { genIslandsLayout } from '../../domain/world-gen/world-gen';
import { AvatarPanelComponent } from '../../ui/avatar-panel/avatar-panel.component';
...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, EducaBadgeComponent, AvatarPanelComponent],
      ...
      <div class="relative w-full h-full flex flex-col bg-[#071326] text-white select-none overflow-hidden">
        ...
        <header class="relative z-20 flex items-center justify-between gap-3 px-4 sm:px-6 py-2.5 bg-[#0a192f]/90 backdrop-blur-md border-b border-white/10 flex-wrap">
          <div class="flex items-center gap-3">
            <a routerLink="/roadmap/student" class="btn btn-ghost btn-xs text-xs font-mono text-gray-300 hover:text-white" title="Volver a mis clases">← Mis clases</a>
            <div>
            ...
            <div class="flex items-center gap-3 flex-wrap sm:flex-nowrap">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1">
                  <app-educa-badge>{{ u.modules.length }} {{ u.modules.length === 1 ? 'torre' : 'torres' }}</app-educa-badge>
                  <app-educa-badge>⭐ {{ visitedOf(u.id) }}/{{ attachmentsOf(u.id) }} estrellas</app-educa-badge>
                </div>
              </div>
...
export class WorldsPageComponent implements AfterViewInit, OnDestroy {
  ...
  protected enter(): void {
    ...
    if (this.mode() === '3d') {
      if (this.archipelago3d.isDockedIn(uid)) {
        this.router.navigate(['/roadmap/play', a.id, uid]);
      } else {
