...
import { Progress, Section } from '../../data-access/roadmap/roadmap.models';
import { World3dPageComponent } from './world-3d-page.component';
import { VerticalChallenge } from '../../domain/vertical-world/vertical-world.engine';
...
describe('Confirmed progress celebrations', () => {
  ...
  async function setup(completed = false, optional = false) {
    ...
    const save = vi.fn();
    await TestBed.configureTestingModule({ imports: [World3dPageComponent], providers: [provideRouter([]),
      { provide: RoadmapStore, useValue: { progress, sections: signal([unit]), sectionById: () => unit, addProgress: save } },
    ] }).compileComponents();
    const fixture = TestBed.createComponent(World3dPageComponent);
