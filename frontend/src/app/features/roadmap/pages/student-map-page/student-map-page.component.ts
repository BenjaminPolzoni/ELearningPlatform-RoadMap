...
import { InventoryModalComponent, InventoryMode } from '../../ui/inventory-modal/inventory-modal.component';
import { LivesComponent } from '../../ui/lives/lives.component';
import { FIRE_COLORS, FIRE_GRID } from '../../ui/streak/streak.component';
...
import { NODE_TYPE_LABEL, ROLE_LABEL } from '../../ui/labels';
 ...
 */
@Component({
  selector: 'app-student-map-page',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [AvatarSpriteComponent, RouterLink, InventoryModalComponent, RankingPanelComponent, LivesComponent, PixelIconComponent],
        ...
        <header class="flex-shrink-0 relative z-10">
          <nav class="navbar flex h-12 flex-shrink-0 items-center justify-between gap-4 px-4 border-b-4 border-primary bg-gradient-to-r from-[#1A1438] via-[#130E24] to-[#1A1438]">
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <app-avatar-sprite [config]="avatarSrv.avatar()" [height]="48" />
              </a>
            </div>
          </nav>
        </header>
                ...
                @if (avatarIsland(); as island) {
                  <g class="anim-float pointer-events-none">
                    <foreignObject
                      ...
                      height="70"
                    >
                      <app-avatar-sprite [config]="avatarSrv.avatar()" [height]="64" [shadow]="true" />
                    </foreignObject>
                  </g>
                }
              </svg>
...
export class StudentMapPageComponent implements OnDestroy {
    ...
    for (let idx = iOrigin + step; ; idx += step) {
      ...
      this.jumpTimers.push(
        setTimeout(() => {
          ...
          if (isLast) onReach?.();
        }, jump * StudentMapPageComponent.JUMP_MS),
      );
      if (isLast) break;
    }
  }
  ...
  protected enter(i: Island): void {
    this.router.navigate(['/roadmap/student/section', i.u.id]);
  }
