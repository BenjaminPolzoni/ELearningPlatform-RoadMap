...
import { SyncChannelService } from '../../data-access/educa/sync-channel.service';
import { VisitService } from '../../data-access/educa/visit.service';
import { ThemeService } from '../../data-access/theme/theme.service';
...
import {
  ...
  type ModulePlaced,
  type WorldLayout,
} from '../../domain/world-gen/world-gen';
import { genQuestion, type Question } from '../../domain/quiz/math';
...
import { DungeonShopModalComponent } from '../../ui/dungeon-shop-modal/dungeon-shop-modal.component';

@Component({
  selector: 'app-hex-world-page',
  standalone: true,
  ...
  template: `
    @if (missing()) {
      <div class="p-6 text-center text-white">
        ...
        <a routerLink="/roadmap/student" class="btn btn-sm btn-primary mt-3 ui-font text-[9px]">← Volver a Mis clases</a>
      </div>
    } @else {
      <div class="relative h-[calc(100dvh-57px)] w-full overflow-hidden"
        ...
        <div class="absolute left-2 right-2 top-2 flex flex-wrap items-center gap-2 rounded-xl bg-black/60 p-2 text-sm text-white backdrop-blur border border-white/10 shadow-lg">
          <a routerLink="/roadmap/student" class="btn btn-xs btn-outline btn-accent ui-font text-[8px]" title="Volver a Mis clases">← Mis clases</a>
          @if (aid()) {
            <a [routerLink]="['/roadmap/play', aid()]" class="btn btn-xs btn-ghost ui-font text-[8px]">Mundos</a>
          }
        ...
        @if (atCastle()) {
          <aside class="absolute right-2 top-16 w-72 rounded-xl bg-[#1C1E2B] border border-primary/40 p-4 text-center text-white shadow-2xl chamfer" aria-live="polite">
            @if (allComplete()) {
              ...
              <a routerLink="/roadmap/student" class="btn btn-primary btn-sm mt-3 w-full ui-font text-[8px]">← MIS CLASES</a>
            } @else {
              <p class="text-3xl">🔒</p>
...
export class HexWorldPageComponent implements AfterViewInit, OnDestroy {
  ...
  private handleDeletedSection(): void {
    ...
    setTimeout(() => {
      this.router.navigate(['/roadmap/play', this.aid()]);
    }, 1400);
  }
