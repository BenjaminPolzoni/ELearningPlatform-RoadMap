 ...
 * `/roadmap/play/:id` (one card per section with its biome) and from there to the hexagonal
 * world of each section (`/roadmap/play/:id/:sectionId`).
 */
@Component({
    ...
    <div class="mx-auto max-w-3xl p-6 min-h-screen overflow-y-auto">
      ...
      <div class="mt-6 grid gap-4">
        @for (c of courses(); track c.id) {
          <a [routerLink]="['/roadmap/play', c.id]">
            <app-educa-card>
              <div class="flex items-center gap-3">
                ...
                <app-educa-badge>{{ c.sections.length }} {{ c.sections.length === 1 ? 'unidad' : 'unidades' }}</app-educa-badge>
              </div>
            </app-educa-card>
          </a>
        } @empty {
