...
import type { Biome } from '../domain/world-gen/world-gen';
...
export class Archipelago3dService {
  ...
  private setupBoat(): void {
    ...
    // Load 3D model ship-large.glb (src/assets/roadmap/world-3d/Assets/Vehicles/ship-large.glb)
    this.loadShipModel();
  }
  ...
  private async loadShipModel(): Promise<void> {
    try {
      ...
      const colormapTex = await new Promise<THREE.Texture | null>((resolve) => {
        textureLoader.load(
          '/assets/roadmap/world-3d/Assets/Vehicles/Textures/colormap.png',
          (tex) => {
      ...
      const gltf = await this.loadGltfSafe(
        '/assets/roadmap/world-3d/Assets/Vehicles/ship-large.glb',
        '/assets/roadmap/world-3d/Assets/Vehicles/ship-large.glb',
      );
      if (!this.running || !this.scene) return;
  ...
  private async loadWildlifeModels(): Promise<void> {
    ...
    try {
      const dolphinGltf = await this.loadGltfSafe('/assets/roadmap/world-3d/Dolphin.glb', '/assets/roadmap/world-3d/dolphin.glb');
      const dolphinClip =
    ...
    try {
      const whaleGltf = await this.loadGltfSafe('/assets/roadmap/world-3d/Whale.glb', '/assets/roadmap/world-3d/whale.glb');
      const whaleClip =
  ...
  private async loadVolcanoModel(): Promise<void> {
    try {
      const gltf = await this.loadGltfSafe('/assets/roadmap/world/Hexagon/Assets/gltf/decoration/nature/volcano.glb');
