...
import type { Avatar } from '../domain/world-gen/world-gen';
  ...
  private async loadHeraldicWallEmblem(x: number, y: number, z: number): Promise<void> {
    try {
      const [shieldGltf, swordGltf] = await Promise.all([
        this.gltfLoader.loadAsync('/assets/roadmap/world-3d/Assets/CharacterV2/Assets/gltf/shield_badge_color.gltf'),
        this.gltfLoader.loadAsync('/assets/roadmap/world-3d/Assets/CharacterV2/Assets/gltf/sword_1handed.gltf'),
      ]);
  ...
  private async loadVendor(): Promise<void> {
    try {
      const [vendorGltf, animsGltf] = await Promise.all([
        this.gltfLoader.loadAsync('/assets/roadmap/world/characters/Rogue_Hooded.glb'),
        this.gltfLoader.loadAsync('/assets/roadmap/world/characters/Rig_Medium_General.glb'),
      ]);
  ...
  private async loadPlayer(avatarSpec?: Avatar | AvatarBuild): Promise<void> {
    try {
      const [movementAnims, generalAnims] = await Promise.all([
        this.gltfLoader.loadAsync('/assets/roadmap/world/characters/Rig_Medium_MovementBasic.glb'),
        this.gltfLoader.loadAsync('/assets/roadmap/world/characters/Rig_Medium_General.glb'),
      ]);
      ...
      if (avatarSpec && typeof avatarSpec !== 'string') {
        ...
        const gltf = await this.gltfLoader.loadAsync(`/assets/roadmap/world/characters/${charName}.glb`);
