...
export class CharacterController {
  ...
  async loadAnimations(): Promise<void> {
    try {
      const [mclips, gclips] = await Promise.all([
        this.assets.loadClips('/assets/roadmap/world/characters/Rig_Medium_MovementBasic.glb'),
        this.assets.loadClips('/assets/roadmap/world/characters/Rig_Medium_General.glb'),
      ]);
      this.walkClip =
  ...
  async setAvatar(avatar: Avatar | AvatarBuild, scene: THREE.Scene): Promise<void> {
    ...
    if (typeof avatar === 'string') {
      g = await this.assets.load(`/assets/roadmap/world/characters/${avatar}.glb`);
      this.body = g;
