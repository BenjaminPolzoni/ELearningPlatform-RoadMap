...
export const COSMETICS_CATALOG = {
  back: [
    ...
    { id: 'laptop_bag', name: '💼 Bolso Notebook' },
    { id: 'keyboard_back', name: '⌨️ Teclado Gamer', hasRGB: true },
    { id: 'giant_usb', name: '💾 USB Gigante' },
    { id: 'quiver', name: '🏹 Carcaj' },
    { id: 'Skeleton_Quiver.gltf', name: '🏹 Carcaj Esquelético' },
  ],
  head: [
    { id: 'none', name: '🚫 Ninguno' },
...
// -------------------------------------------------------------
    ...
    function createLaptopBag() {
      const group = new THREE.Group();
      group.name = 'laptop_bag';

      const bagMat = new THREE.MeshStandardMaterial({ color: 0x1f2937, roughness: 0.5, metalness: 0.2 });
      const flapMat = new THREE.MeshStandardMaterial({ color: 0x111827, roughness: 0.4, metalness: 0.3 });
      const strapMat = new THREE.MeshStandardMaterial({ color: 0x374151, roughness: 0.8 });
      const buckleMat = new THREE.MeshStandardMaterial({ color: 0xe2e8f0, metalness: 0.9, roughness: 0.2 });
      const accentMat = new THREE.MeshStandardMaterial({ color: 0x38bdf8, emissive: 0x0284c7, emissiveIntensity: 0.5 });

      // Main messenger bag body (positioned lower and tilted)
      const bag = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.36, 0.12), bagMat);
      bag.position.set(0.05, -0.10, -0.08);
      bag.castShadow = true;
      group.add(bag);

      // Front flap cover
      const flap = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.24, 0.13), flapMat);
      flap.position.set(0.05, -0.04, -0.08);
      group.add(flap);

      // Neon tech accent stripe
      const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.018, 0.135), accentMat);
      stripe.position.set(0.05, -0.14, -0.08);
      group.add(stripe);

      // Dual metal buckles
      for (const x of [-0.10, 0.20]) {
        const buckle = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.06, 0.14), buckleMat);
        buckle.position.set(x, -0.15, -0.08);
        group.add(buckle);
      }

      // Crossbody diagonal strap that passes over the right shoulder and loops around torso!
      const strapCurve = new THREE.CatmullRomCurve3([
        new THREE.Vector3(-0.16, -0.15, -0.08),
        new THREE.Vector3(-0.25, 0.15, 0.10),
        new THREE.Vector3(-0.10, 0.28, 0.22),
        new THREE.Vector3(0.18, 0.42, 0.14),
        new THREE.Vector3(0.24, 0.20, -0.10),
        new THREE.Vector3(0.20, -0.05, -0.12),
        new THREE.Vector3(0.12, -0.10, -0.08)
      ], true);

      const strapTube = new THREE.Mesh(new THREE.TubeGeometry(strapCurve, 24, 0.035, 6, true), strapMat);
      group.add(strapTube);
    ...
    function createPropellerHat() {
      ...
      // 2. Visera anatómica curva bien metida dentro de la cúpula (sin bordes cortados ni planos triangulares)
      const visorShape = new THREE.Shape();
      // Comienza embutida en el interior del gorro (Z = 0.36)
      visorShape.moveTo(0.26, 0.36);
      // Curva lateral derecha hacia la esquina frontal
      visorShape.bezierCurveTo(0.28, 0.48, 0.26, 0.58, 0.20, 0.64);
      // Frente redondeado continuo
      visorShape.bezierCurveTo(0.12, 0.69, -0.12, 0.69, -0.20, 0.64);
      // Curva lateral izquierda
      visorShape.bezierCurveTo(-0.26, 0.58, -0.28, 0.48, -0.26, 0.36);
      // Cierre posterior interno oculto dentro de la esfera
      visorShape.bezierCurveTo(-0.13, 0.39, 0.13, 0.39, 0.26, 0.36);
      ...
      const brimGeom = new THREE.ExtrudeGeometry(visorShape, extrudeSettings);
      brimGeom.rotateX(Math.PI / 2); // +Y en 2D pasa a ser +Z (frente) en 3D

      // Arqueo sutil lateral para dar curvatura realista de gorra deportiva
      const pos = brimGeom.attributes.position;
      for (let i = 0; i < pos.count; i++) {
        const x = pos.getX(i);
        pos.setY(i, pos.getY(i) - 0.22 * x * x);
      }
      pos.needsUpdate = true;
      brimGeom.computeVertexNormals();

      const brim = new THREE.Mesh(brimGeom, blueMat);
      brim.position.set(0, capY + 0.01, 0);
      brim.rotation.x = 0.14;
      group.add(brim);

      // 3. Arandela base superior donde nace el eje
    ...
    function createFlyingOwl() {
      ...
      const eyeMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, emissive: 0xd97706, emissiveIntensity: 0.8 });
      const pupilMat = new THREE.MeshBasicMaterial({ color: 0x09090b });
      const beakMat = new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.4 });
      const wingMat = new THREE.MeshStandardMaterial({ color: 0x451a03, roughness: 0.7 });

      // Body egg
      const body = new THREE.Mesh(new THREE.SphereGeometry(0.10, 16, 14), bodyMat);
      body.scale.set(0.9, 1.1, 0.9);
      group.add(body);

      // Cream chest
      const chest = new THREE.Mesh(new THREE.SphereGeometry(0.08, 14, 12), chestMat);
      chest.position.set(0, -0.02, 0.04);
      chest.scale.set(0.8, 0.9, 0.7);
      group.add(chest);

      // Beak
      const beak = new THREE.Mesh(new THREE.ConeGeometry(0.022, 0.05, 8), beakMat);
      beak.rotation.x = Math.PI / 2;
      beak.position.set(0, 0.02, 0.10);
      group.add(beak);

      // Eyes
      for (const side of [-1, 1]) {
        const eyeRing = new THREE.Mesh(new THREE.SphereGeometry(0.038, 12, 12), chestMat);
        eyeRing.position.set(side * 0.045, 0.05, 0.07);
        group.add(eyeRing);

        const eye = new THREE.Mesh(new THREE.SphereGeometry(0.026, 12, 12), eyeMat);
        eye.position.set(side * 0.045, 0.05, 0.085);
        group.add(eye);

        const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.014, 8, 8), pupilMat);
        pupil.position.set(side * 0.045, 0.05, 0.102);
        group.add(pupil);

        // Ear tufts
        const tuft = new THREE.Mesh(new THREE.ConeGeometry(0.02, 0.06, 6), bodyMat);
        tuft.position.set(side * 0.05, 0.13, 0);
      ...
      wingL.position.set(-0.09, 0.03, 0);
      const wingMeshL = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.02, 0.12), wingMat);
      wingMeshL.position.set(-0.08, 0, 0);
      wingL.add(wingMeshL);
      group.add(wingL);

      const wingR = new THREE.Group();
      wingR.position.set(0.09, 0.03, 0);
      const wingMeshR = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.02, 0.12), wingMat);
      wingMeshR.position.set(0.08, 0, 0);
      wingR.add(wingMeshR);
      group.add(wingR);

      group.userData.updateAnim = (time, delta) => {
        const flap = Math.sin(time * 9) * 0.55;
    ...
    function createFlyingGhost() {
      ...
      const faceMat = new THREE.MeshBasicMaterial({ color: 0x0369a1 });

      // Ghost head hemisphere
      const head = new THREE.Mesh(new THREE.SphereGeometry(0.09, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.5), ghostMat);
      group.add(head);

      // Ghost skirt cylinder
      const skirt = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.06, 0.14, 16, 1, true), ghostMat);
      skirt.position.set(0, -0.07, 0);
      group.add(skirt);

      // Wavy tail
      const tail = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.12, 12), ghostMat);
      ...
      // Eyes & arms
      for (const side of [-1, 1]) {
        const eye = new THREE.Mesh(new THREE.SphereGeometry(0.016, 8, 8), faceMat);
        eye.position.set(side * 0.035, -0.01, 0.085);
        group.add(eye);

        const arm = new THREE.Mesh(new THREE.SphereGeometry(0.025, 8, 8), ghostMat);
        arm.position.set(side * 0.09, -0.05, 0.02);
        group.add(arm);
      }

      // Open mouth
      const mouth = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.01, 8), faceMat);
      ...
      group.userData.updateAnim = (time, delta) => {
        ...
        ghostMat.emissiveIntensity = glow;
        tail.rotation.z = Math.sin(time * 5) * 0.25;
        tail.rotation.x = Math.PI * 0.75 + Math.cos(time * 4) * 0.15;
      };
    ...
    function updateFlyingPet(time, delta) {
      ...
      const y = height + Math.sin(time * 3.5) * 0.05;
      activePetObject.position.set(x, y, z);
      activePetObject.rotation.y = -angle + Math.PI / 2;
